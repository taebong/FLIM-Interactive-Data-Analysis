function varargout = KinetochoreSelectionTool_ver9_1(varargin)
% KINETOCHORESELECTIONTOOL_VER9_1 MATLAB code for KinetochoreSelectionTool_ver9_1.fig
%      KINETOCHORESELECTIONTOOL_VER9_1, by itself, creates a new KINETOCHORESELECTIONTOOL_VER9_1 or raises the existing
%      singleton*.
%
%      H = KINETOCHORESELECTIONTOOL_VER9_1 returns the handle to a new KINETOCHORESELECTIONTOOL_VER9_1 or the handle to
%      the existing singleton*.
%
%      KINETOCHORESELECTIONTOOL_VER9_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KINETOCHORESELECTIONTOOL_VER9_1.M with the given input arguments.
%
%      KINETOCHORESELECTIONTOOL_VER9_1('Property','Value',...) creates a new KINETOCHORESELECTIONTOOL_VER9_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KinetochoreSelectionTool_ver9_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KinetochoreSelectionTool_ver9_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help KinetochoreSelectionTool_ver9_1
% Last Modified by GUIDE v2.5 12-Apr-2017 13:59:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @KinetochoreSelectionTool_ver9_1_OpeningFcn, ...
    'gui_OutputFcn',  @KinetochoreSelectionTool_ver9_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function varargout = KinetochoreSelectionTool_ver9_1_OutputFcn(hObject, eventdata, handles)
...
% --- Executes just before KinetochoreSelectionTool_ver9_1 is made visible.
function KinetochoreSelectionTool_ver9_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KinetochoreSelectionTool_ver9_1 (see VARARGIN)

% Choose default command line output for KinetochoreSelectionTool_ver9_1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes KinetochoreSelectionTool_ver9_1 wait for user response (see UIRESUME)
...
ch1 = get(handles.Ch1_checkbox,'Value');
ch2 = get(handles.Ch2_checkbox,'Value');
trackbool = get(handles.TrackKinetochores_checkbox,'Value');
...
if Nframe==1
    ...
    if trackbool %get kinetochore tracks
        ...
        res(:,1:7,:)=MT;
        colkinnum = (1:size(MT,1))';
        colkinnum = arrayfun(@(t) t-nnz(MT(:,6,1)<=(MT(t,6,1)-1) & colkinnum<=t),colkinnum);
        res(:,8,1) = colkinnum;
        res(:,8,2) = colkinnum;
        res(:,:,1) = sortrows(res(:,:,1),6);
        res(:,:,2) = sortrows(res(:,:,2),6);
    end
...
if isnan(kin_selected)
    error('No kinetochore selected');
    return;
end
...
for k = 1:Nkinselected
    for fr = 1:Nframe
        image_struct = MainGuihandles.image_struct{selected(fr)};
...
% --- Executes on button press in TrackKinetochores_checkbox.
function TrackKinetochores_checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to TrackKinetochores_checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of TrackKinetochores_checkbox
