function [p,X2,sigma_p,sigma_y,corr,R_sq,cvg_hst, converged] = lm2_v01(func,p,x,y,data,weight,dp,p_min,p_max,c)

% [p,X2,sigma_p,sigma_y,corr,R_sq,cvg_hst] = lm(func,p,X,data,weight,dp,p_min,p_max,c)
%
% Levenberg Marquardt curve-fitting: minimize sum of weighted squared residuals
% -------- INPUT VARIABLES ---------
% func   = function of n1 independent variables, X{1} and n2 X{2}, and m parameters, 'p',
%          returning the simulated model: y_hat = func(t,p,c)
% p      = m-vector of initial guess of parameter values
% X   = cell array of n1 and n2-vectors of independent variables (used as arg to func)
% y_dat  = n2xn1 matrix of data to be fit by func(x,y,p)
...
%   modified by Tae Yeon Yoo in Needleman Group for 2d gaussian fitting.
...
if length(y)~= size(data,1) & length(x)~=size(data,2)
    disp('lm.m error: the size of x and y must match to the size of data');
...
prnt = 3;		% >1 intermediate results; >2 plots  %plotting option not available
...
if nargin < 6, weight = sqrt((Npnt-Npar+1)/(y_dat.^2)); end
if nargin < 7, dp = 0.001; end
if nargin < 8, p_min   = -100*abs(p); end
if nargin < 9, p_max   =  100*abs(p); end
if nargin < 10, c       =  1; end
...
[alpha,beta,X2,y_hat,dydp] = lm_matx(func,x,y,p,y_dat,weight_sq,dp,c);
...
nonzero_y_dat = y_dat;
nonzero_y_dat(find(y_dat==0))=1;
sigy = sqrt(nonzero_y_dat);
weight_sq = 1./sigy.^2;
...
function dydp = lm_dydp(func,x,y,p,ydat,dp,c)
% dydp = lm_dydp(func,x,y,p,y,{dp},{c})
%
...
m=length(ydat);			% number of data points
...
function [alpha,beta,Chi_sq,y_hat,dydp] = lm_matx(func,x,y,p,y_dat,weight_sq,dp,c)
...
y_hat = feval(func,x,y,p,c);	% evaluate model using parameters 'p'
y_hat = y_hat(:);
func_calls = func_calls + 1;

delta_y = y_dat - y_hat;	% residual error between model and data

dydp = lm_dydp(func,x,y,p,y_hat,dp,c);
