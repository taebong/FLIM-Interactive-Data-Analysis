function varargout = FittingGUI_ver2_2(varargin)
% FITTINGGUI_VER2_2 MATLAB code for FittingGUI_ver2_2.fig
%      FITTINGGUI_VER2_2, by itself, creates a new FITTINGGUI_VER2_2 or raises the existing
%      singleton*.
%
%      H = FITTINGGUI_VER2_2 returns the handle to a new FITTINGGUI_VER2_2 or the handle to
%      the existing singleton*.
%
%      FITTINGGUI_VER2_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FITTINGGUI_VER2_2.M with the given input arguments.
%
%      FITTINGGUI_VER2_2('Property','Value',...) creates a new FITTINGGUI_VER2_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FittingGUI_ver2_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FittingGUI_ver2_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FittingGUI_ver2_2

% Last Modified by GUIDE v2.5 15-Apr-2013 17:52:04

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FittingGUI_ver2_2_OpeningFcn, ...
    'gui_OutputFcn',  @FittingGUI_ver2_2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FittingGUI_ver2_2 is made visible.
function FittingGUI_ver2_2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FittingGUI_ver2_2 (see VARARGIN)

% Choose default command line output for FittingGUI_ver2_2
...
% UIWAIT makes FittingGUI_ver2_2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FittingGUI_ver2_2_OutputFcn(hObject, eventdata, handles)
...
elseif fitting_method == 1   % Do LS fitting
    ...
    if nexpo == 1
        ...
        p_max = [1.2*max(decay),5,est_noise+10]';
    elseif nexpo == 2
        p_init = [0.5*max(decay),3,est_noise,0.5*max(decay),0.7];
        p_min = [0.2*max(decay),0.05,0.01,1,0.05]';
        p_max = [1.1*max(decay),5,est_noise+10,1.1*max(decay),5]';
    elseif nexpo == 3
    ...
    decay_to_fit{selected}.cvg_hst = cvg_hst;
