function varargout = KinetochoreSelectionTool_ver7_1(varargin)
% KINETOCHORESELECTIONTOOL_VER7_1 MATLAB code for KinetochoreSelectionTool_ver7_1.fig
%      KINETOCHORESELECTIONTOOL_VER7_1, by itself, creates a new KINETOCHORESELECTIONTOOL_VER7_1 or raises the existing
%      singleton*.
%
%      H = KINETOCHORESELECTIONTOOL_VER7_1 returns the handle to a new KINETOCHORESELECTIONTOOL_VER7_1 or the handle to
%      the existing singleton*.
%
%      KINETOCHORESELECTIONTOOL_VER7_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KINETOCHORESELECTIONTOOL_VER7_1.M with the given input arguments.
%
%      KINETOCHORESELECTIONTOOL_VER7_1('Property','Value',...) creates a new KINETOCHORESELECTIONTOOL_VER7_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KinetochoreSelectionTool_ver7_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KinetochoreSelectionTool_ver7_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help KinetochoreSelectionTool_ver7_1
% Last Modified by GUIDE v2.5 21-Aug-2015 14:03:07

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @KinetochoreSelectionTool_ver7_1_OpeningFcn, ...
    'gui_OutputFcn',  @KinetochoreSelectionTool_ver7_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function varargout = KinetochoreSelectionTool_ver7_1_OutputFcn(hObject, eventdata, handles)
...
% --- Executes just before KinetochoreSelectionTool_ver7_1 is made visible.
function KinetochoreSelectionTool_ver7_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KinetochoreSelectionTool_ver7_1 (see VARARGIN)

% Choose default command line output for KinetochoreSelectionTool_ver7_1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes KinetochoreSelectionTool_ver7_1 wait for user response (see UIRESUME)
                    ...
                    switch button3
                        case 'Yes'
                            ...
                            if isempty(kinpair) == 0
                                ...
                                %this frame
                                idx = sum(ismember(pairedkin,newid),1)>0;
                                kinpair(find(idx),framenum).kinpairstate = 0;
                                
                                kinpair_handle = kinpair(find(idx),framenum).kinpair_handle;
                                kinpairtext_handle = kinpair(find(idx),framenum).kinpairtext_handle;
        ...
        case 'Delete'
            ...
            switch button5
                case 'Only this frame'
                    ...
                    if isempty(kinpair)==0
                        pairedkin = [kinpair(:,framenum).kinids];
                        kinpair_handle = [kinpair(:,framenum).kinpair_handle];
                        kinpairtext_handle = [kinpair(:,framenum).kinpairtext_handle];
                        
                        %reject the kinpair with the deleted kin id in
                        %this frame
                        idx = sum(ismember(pairedkin,selected_kin),1)>0;
                        kinpair_handle_rm = kinpair_handle(:,idx);
                        kinpairtext_handle_rm = kinpairtext_handle(:,idx);
                        delete(kinpair_handle_rm(ishandle(kinpair_handle_rm)));
                        delete(kinpairtext_handle_rm(ishandle(kinpairtext_handle_rm)));
                ...
                case 'All frames'
                    ...
                    if isempty(kinpair)==0
                        ...
                        if sum(idx)
                            [kinpair(find(idx)).kinpairstate] = deal(0);
                        end
                    end
...
function outkin = rejectKinetochore(handles,kin)
rejectoverlap = get(handles.RejectTrjOverlap_checkbox,'Value');
...
rejectradisu = get(handles.RejectRadius_checkbox,'Value');

distthres = str2double(get(handles.OverlapDistThres_edit,'String'));
