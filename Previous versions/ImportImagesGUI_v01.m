function varargout = ImportImagesGUI_v01(varargin)
% IMPORTIMAGESGUI_V01 MATLAB code for ImportImagesGUI_v01.fig
%      IMPORTIMAGESGUI_V01 by itself, creates a new IMPORTIMAGESGUI_V01 or raises the
%      existing singleton*.
%
%      H = IMPORTIMAGESGUI_V01 returns the handle to a new IMPORTIMAGESGUI_V01 or the handle to
%      the existing singleton*.
%
%      IMPORTIMAGESGUI_V01('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in IMPORTIMAGESGUI_V01.M with the given input arguments.
%
%      IMPORTIMAGESGUI_V01('Property','Value',...) creates a new IMPORTIMAGESGUI_V01 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ImportImagesGUI_v01_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ImportImagesGUI_v01_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ImportImagesGUI_v01

% Last Modified by GUIDE v2.5 01-Apr-2015 16:37:42

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ImportImagesGUI_v01_OpeningFcn, ...
    'gui_OutputFcn',  @ImportImagesGUI_v01_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ImportImagesGUI_v01 is made visible.
function ImportImagesGUI_v01_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ImportImagesGUI_v01 (see VARARGIN)
...
% UIWAIT makes ImportImagesGUI_v01 wait for user response (see UIRESUME)
%uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = ImportImagesGUI_v01_OutputFcn(hObject, eventdata, handles)
...
if strcmp(ext,'sdt')
    ...
    for i = 1:NumOfNewImages
        ...
        image_struct{i}.selected_pixel = zeros(size(img,2),size(img,1));
...
if renamebool
    ...
    sortinfo = MultiD_sdt_sort(pathname,0);
end
