...
function varargout = FLIMDataAnalysisGUI_ver7_3(varargin)
% FLIMDATAANALYSISGUI_VER7_3 MATLAB code for FLIMDataAnalysisGUI_ver7_3.fig
%      FLIMDATAANALYSISGUI_VER7_3, by itself, creates a new FLIMDATAANALYSISGUI_VER7_3 or raises the existing
%      singleton*.
%d
%      H = FLIMDATAANALYSISGUI_VER7_3 returns the handle to a new FLIMDATAANALYSISGUI_VER7_3 or the handle to
%      the existing singleton*.
%
%      FLIMDATAANALYSISGUI_VER7_3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FLIMDATAANALYSISGUI_VER7_3.M with the given input arguments.
%
%      FLIMDATAANALYSISGUI_VER7_3('Property','Value',...) creates a new FLIMDATAANALYSISGUI_VER7_3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FLIMDataAnalysisGUI_ver7_3_OpeningFcn gets called.  An
...
% Edit the above text to modify the response to help FLIMDataAnalysisGUI_ver7_3

% Last Modified by GUIDE v2.5 31-May-2015 16:48:04

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FLIMDataAnalysisGUI_ver7_3_OpeningFcn, ...
    'gui_OutputFcn',  @FLIMDataAnalysisGUI_ver7_3_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FLIMDataAnalysisGUI_ver7_3 is made visible.
function FLIMDataAnalysisGUI_ver7_3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FLIMDataAnalysisGUI_ver7_3 (see VARARGIN)

% Choose default command line output for FLIMDataAnalysisGUI_ver7_3
...
% so window can get raised using FLIMDataAnalysisGUI_ver7_3.
if strcmp(get(hObject,'Visible'),'off')
    
end

% UIWAIT makes FLIMDataAnalysisGUI_ver7_3 wait for user response (see UIRESUME)
% uiwait(handles.MainGui);


% --- Outputs from this function are returned to the command line.
function varargout = FLIMDataAnalysisGUI_ver7_3_OutputFcn(hObject, eventdata, handles)
    ...
    for i = 1:Nframe
        ...
        if showM1 && showM2
            ...
            if i == 1
                ...
                imwrite(img1,nameM1,'writemode','append');
                imwrite(img2,nameM2,'writemode','append');
            end
        else
            if i == 1
                imwrite(img,name);
            else
                imwrite(img,name,'writemode','append');
            end
        end
        for i = 1:10000
        end
...
for i = 1:Nselected
    ind = find(sortinfo.timestamp==timesdt(image_struct(i).setting));
    if isempty(ind)
        newname(i) = 'unsorted';
    else
        matchingind(i) = ind;
