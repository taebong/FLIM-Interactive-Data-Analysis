function varargout = KinetochoreSelectionTool_ver7_2(varargin)
% KINETOCHORESELECTIONTOOL_VER7_2 MATLAB code for KinetochoreSelectionTool_ver7_2.fig
%      KINETOCHORESELECTIONTOOL_VER7_2, by itself, creates a new KINETOCHORESELECTIONTOOL_VER7_2 or raises the existing
%      singleton*.
%
%      H = KINETOCHORESELECTIONTOOL_VER7_2 returns the handle to a new KINETOCHORESELECTIONTOOL_VER7_2 or the handle to
%      the existing singleton*.
%
%      KINETOCHORESELECTIONTOOL_VER7_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KINETOCHORESELECTIONTOOL_VER7_2.M with the given input arguments.
%
%      KINETOCHORESELECTIONTOOL_VER7_2('Property','Value',...) creates a new KINETOCHORESELECTIONTOOL_VER7_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KinetochoreSelectionTool_ver7_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KinetochoreSelectionTool_ver7_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help KinetochoreSelectionTool_ver7_2
% Last Modified by GUIDE v2.5 21-Aug-2015 14:04:42

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @KinetochoreSelectionTool_ver7_2_OpeningFcn, ...
    'gui_OutputFcn',  @KinetochoreSelectionTool_ver7_2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function varargout = KinetochoreSelectionTool_ver7_2_OutputFcn(hObject, eventdata, handles)
...
% --- Executes just before KinetochoreSelectionTool_ver7_2 is made visible.
function KinetochoreSelectionTool_ver7_2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KinetochoreSelectionTool_ver7_2 (see VARARGIN)

% Choose default command line output for KinetochoreSelectionTool_ver7_2
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes KinetochoreSelectionTool_ver7_2 wait for user response (see UIRESUME)
    ...
    switch button
        case 'Change ID'
            ...
            while nopass
                ...
                elseif kin(newid,framenum).kinstate == 1
                    ...
                    switch button3
                        case 'Yes'
                            [kin,frame,kinpair] = rejectKinetochore(kin,frame,kinpair,newid,framenum);
                            nopass = 0;
                        case 'No'
                            return;
                        case 'Cancel'
        ...
        case 'Delete'
            ...
            switch button5
                ...
                case 'All frames'
                    frametodelete = find([kin(selected_kin,:).kinstate]==1);
                case 'Cancel'
                    return
            end
            [kin,frame,kinpair] = rejectKinetochore(kin,frame,kinpair,selected_kin,frametodelete);
...
set(handles.KPair_listbox,'String',contents);
...
for k = 1:N_rm
    [kin,frame,kinpair] = rejectKinetochore(kin,frame,kinpair,kin_selected(k),1:length(frame));
...
hMainGui = getappdata(0, 'hMainGui');
if isempty(hMainGui)
    errordlg('Not linked to FLIM Data Analysis Main GUI')
    return;
end
...
if name ~= 0
    save([pathname,name],'ROI','kin','kinpair','frame','spaxis');
end
    ...
    if showkin
        for kinid = 1:Nkin
            for ch = 1:2
                ...
                if source == -1   %from center of mass
                    color = 'g';
                    cenx = kin(kinid,fr).x(ch);
                    ceny = kin(kinid,fr).y(ch);
                    Rcover = kin(kinid,fr).Rg(ch)*coverage;
                elseif source == 0
                    color = 'r';
                elseif source == 1
                    color = 'b';
                ...
                if source == 0 | source ==1  % from Gauss fit
                    cenx = kin(kinid,fr).mux(ch);
                    ceny = kin(kinid,fr).muy(ch);
                    ra = kin(kinid,fr).sigx(ch)*coverage;
                    rb = kin(kinid,fr).sigy(ch)*coverage;
                    theta = kin(kinid,fr).theta;
                end
                ...
                ind = frame{fr}.kin_id == kinid;
                
                if source == -1
                    %(Re)draw kinetochore circles
                    DrawCircle(cenx,ceny,Rcover,color);
                else
                    %Draw ellipse
                    kincircle_handle = ellipse(ra,rb,theta,cenx,ceny,color);
                    set(kincircle_handle,'LineWidth',1.5);
...
kin = handles.kin;
kin = rejectKinetochoreUsingCriteria(handles,kin);
...
function outkin = rejectKinetochoreUsingCriteria(handles,kin)
...
rejectellise = get(handles.RejectEllipticity_checkbox,'Value');
...
if rejectoverlap
    for fr = 1:Nframe
        for ch = 1:2
            ...
            radius = ((locsource(ch,:)==-1).*Rg(ch,:)+(locsource(ch,:)>=0).*(sigx(ch,:)+sigy(ch,:))/2)*coverage;
...
function [outkin,outframe,outkinpair] = rejectKinetochore(kin,frame,kinpair,kinid,frameid)
...
for j = 1:Nframeid
    ...
    tempind = find(frame{fr}.kin_id == kinid);
    frame{fr}.kin_id(tempind) = [];
    if sum(ishandle(frame{fr}.text_handle(tempind,:)))
        delete(frame{fr}.text_handle(tempind,ishandle(frame{fr}.text_handle(tempind,:))))
    end
    frame{fr}.text_handle(tempind,:) = [];
    if sum(ishandle(frame{fr}.kincircle_handle(tempind,:)))
        delete(frame{fr}.kincircle_handle(tempind,ishandle(frame{fr}.kincircle_handle(tempind,:))))
    end
    frame{fr}.kincircle_handle(tempind,:) = [];
