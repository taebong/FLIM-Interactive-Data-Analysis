function varargout = FittingGUI_ver8_1(varargin)
% FITTINGGUI_VER8_1 MATLAB code for FittingGUI_ver8_1.fig
%      FITTINGGUI_VER8_1, by itself, creates a new FITTINGGUI_VER8_1 or raises the existing
%      singleton*.
%
%      H = FITTINGGUI_VER8_1 returns the handle to a new FITTINGGUI_VER8_1 or the handle to
%      the existing singleton*.
%
%      FITTINGGUI_VER8_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FITTINGGUI_VER8_1.M with the given input arguments.
%
%      FITTINGGUI_VER8_1('Property','Value',...) creates a new FITTINGGUI_VER8_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FittingGUI_ver8_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FittingGUI_ver8_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FittingGUI_ver8_1

% Last Modified by GUIDE v2.5 25-Feb-2015 17:20:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FittingGUI_ver8_1_OpeningFcn, ...
    'gui_OutputFcn',  @FittingGUI_ver8_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FittingGUI_ver8_1 is made visible.
function FittingGUI_ver8_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FittingGUI_ver8_1 (see VARARGIN)

% Choose default command line output for FittingGUI_ver8_1
...
% UIWAIT makes FittingGUI_ver8_1 wait for user response (see UIRESUME)
% uiwait(handles.FittingGui);


% --- Outputs from this function are returned to the command line.
function varargout = FittingGUI_ver8_1_OutputFcn(hObject, eventdata, handles)
...
[decay_handle,fit_handle,residual_handle] = getDecayHandles(decay_struct);
...
[decay_handle,fit_handle,residual_handle] = getDecayHandles(dstructlist);
...
for j = selected
    if fitting_method == 2 %Gibbs
        answer = inputdlg({'Number of samples','Number of burning samples'},'Gibbs Sampling Setting',1,{'5000','100'});
    ...
    decay_struct{j} = DecayFLIMFit(decay_struct{j},...
        fitting_method,nexpo,fitvar,fit_start,fit_end,noise_region_from,noise_region_to,prior,Nsample,Nburn);
...
for i = 1:Nselected
    matchingind(i) = find(sortinfo.timestamp==timesdt(decay_struct(i).setting));
