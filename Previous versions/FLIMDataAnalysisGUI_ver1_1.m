function varargout = FLIMDataAnalysisGUI_ver1_1(varargin)
% FLIMDATAANALYSISGUI_VER1_1 MATLAB code for FLIMDataAnalysisGUI_ver1_1.fig
%      FLIMDATAANALYSISGUI_VER1_1, by itself, creates a new FLIMDATAANALYSISGUI_VER1_1 or raises the existing
%      singleton*.
%
%      H = FLIMDATAANALYSISGUI_VER1_1 returns the handle to a new FLIMDATAANALYSISGUI_VER1_1 or the handle to
%      the existing singleton*.
%
%      FLIMDATAANALYSISGUI_VER1_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FLIMDATAANALYSISGUI_VER1_1.M with the given input arguments.
%
%      FLIMDATAANALYSISGUI_VER1_1('Property','Value',...) creates a new FLIMDATAANALYSISGUI_VER1_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FLIMDataAnalysisGUI_ver1_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FLIMDataAnalysisGUI_ver1_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FLIMDataAnalysisGUI_ver1_1

% Last Modified by GUIDE v2.5 19-Feb-2013 17:39:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FLIMDataAnalysisGUI_ver1_1_OpeningFcn, ...
    'gui_OutputFcn',  @FLIMDataAnalysisGUI_ver1_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FLIMDataAnalysisGUI_ver1_1 is made visible.
function FLIMDataAnalysisGUI_ver1_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FLIMDataAnalysisGUI_ver1_1 (see VARARGIN)

% Choose default command line output for FLIMDataAnalysisGUI_ver1_1
...
% so window can get raised using FLIMDataAnalysisGUI_ver1_1.
if strcmp(get(hObject,'Visible'),'off')
    
end


% UIWAIT makes FLIMDataAnalysisGUI_ver1_1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FLIMDataAnalysisGUI_ver1_1_OutputFcn(hObject, eventdata, handles)
...
image_handle = handles.iamge.image_handle(selected_image);
NumOfImages = handles.image.NumOfImages;

%delete information related to the current image
delete(image_handle);
delete(selected_pixel_handle);
delete(decay_handle);
handles.image.image_handle(selected_image) = [];
handles.image.selected_pixel(selected_image) = [];
handles.image.selected_pixel_handle(selected_image) = [];
handles.image.decay(selected_image) = [];
handles.image.decay_handle(selected_image) = [];

axes(handles.axes1);
if NumOfImages==1
    cla;
    %update image selection slider
    set(handles.slider1,'Max',2);
    set(handles.slider1,'Visible','off');
else
    handles.image.NumOfImages = NumOfImages-1;
    
    if selected_image > 1
        set(handles.image.image_handle(selected_image-1),'Visible','on');
        set(handles.image.selected_pixel_handle(selected_image-1),'Visible','on');
        
        axes(handles.axes2);
        set(handles.image.decay_handle(selected_image-1),'Visible','on');
    else
    end
%Show filename
