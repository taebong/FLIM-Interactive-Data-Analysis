function varargout = KinetochoreSelectionTool_ver6_1(varargin)
% KINETOCHORESELECTIONTOOL_VER6_1 MATLAB code for KinetochoreSelectionTool_ver6_1.fig
%      KINETOCHORESELECTIONTOOL_VER6_1, by itself, creates a new KINETOCHORESELECTIONTOOL_VER6_1 or raises the existing
%      singleton*.
%
%      H = KINETOCHORESELECTIONTOOL_VER6_1 returns the handle to a new KINETOCHORESELECTIONTOOL_VER6_1 or the handle to
%      the existing singleton*.
%
%      KINETOCHORESELECTIONTOOL_VER6_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KINETOCHORESELECTIONTOOL_VER6_1.M with the given input arguments.
%
%      KINETOCHORESELECTIONTOOL_VER6_1('Property','Value',...) creates a new KINETOCHORESELECTIONTOOL_VER6_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KinetochoreSelectionTool_ver6_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KinetochoreSelectionTool_ver6_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help KinetochoreSelectionTool_ver6_1
% Last Modified by GUIDE v2.5 13-Aug-2015 10:40:09

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @KinetochoreSelectionTool_ver6_1_OpeningFcn, ...
    'gui_OutputFcn',  @KinetochoreSelectionTool_ver6_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function varargout = KinetochoreSelectionTool_ver6_1_OutputFcn(hObject, eventdata, handles)
...
% --- Executes just before KinetochoreSelectionTool_ver6_1 is made visible.
function KinetochoreSelectionTool_ver6_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KinetochoreSelectionTool_ver6_1 (see VARARGIN)

% Choose default command line output for KinetochoreSelectionTool_ver6_1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes KinetochoreSelectionTool_ver6_1 wait for user response (see UIRESUME)
...
if eventData.Button == 3
    ...
    distance = sqrt(([kin(kinids,framenum).x]-coord(1)).^2+...
        ([kin(kinids,framenum).y]-coord(2)).^2);
...
switch button
    ...
    case 'Delete'
        ...
        switch button5
            ...
            case 'All frames'
                kinlist = str2double(get(handles.Kinetochore_listbox,'String'));
                newval = find(kinlist==selected_kin);
                set(handles.Kinetochore_listbox,'Value',newval);
                RemoveSelected_pushbutton_Callback(handles.RemoveSelected_pushbutton,newval,handles)
    ...
    for fr = 1:Nframe
        for kinid = 1:Nkin
            for ch = 1:2
                ...
                elseif kin(kinid,fr).converged(ch) == 0 && inclduenonconverged == 0
