function varargout = FittingGUI_ver4_3(varargin)
% FITTINGGUI_VER4_3 MATLAB code for FittingGUI_ver4_3.fig
%      FITTINGGUI_VER4_3, by itself, creates a new FITTINGGUI_VER4_3 or raises the existing
%      singleton*.
%
%      H = FITTINGGUI_VER4_3 returns the handle to a new FITTINGGUI_VER4_3 or the handle to
%      the existing singleton*.
%
%      FITTINGGUI_VER4_3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FITTINGGUI_VER4_3.M with the given input arguments.
%
%      FITTINGGUI_VER4_3('Property','Value',...) creates a new FITTINGGUI_VER4_3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FittingGUI_ver4_3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FittingGUI_ver4_3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FittingGUI_ver4_3

% Last Modified by GUIDE v2.5 10-Sep-2014 15:37:04

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FittingGUI_ver4_3_OpeningFcn, ...
    'gui_OutputFcn',  @FittingGUI_ver4_3_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FittingGUI_ver4_3 is made visible.
function FittingGUI_ver4_3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FittingGUI_ver4_3 (see VARARGIN)

% Choose default command line output for FittingGUI_ver4_3
...
if (isempty(mainGuiInput)) ...
    ...
    if strcmp(mode,'Append') == 1
        NumOfDecays = handles.NumOfDecays;
        NumOfDecays = NumOfDecays + length(decay_struct);
    end
    
    NumOfDecays = length(decay_struct);
    
    for i = 1:NumOfDecays
        decay_struct{i} = DecayStructInitializer(new_decay_struct{i});
    end
end
...
% UIWAIT makes FittingGUI_ver4_3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FittingGUI_ver4_3_OutputFcn(hObject, eventdata, handles)
...
for j = selected
    ...
    decay_struct{j}.residual_handle = plot(time,residual);
