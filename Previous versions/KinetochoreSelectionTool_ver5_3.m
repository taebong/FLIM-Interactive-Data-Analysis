function varargout = KinetochoreSelectionTool_ver5_3(varargin)
% KINETOCHORESELECTIONTOOL_VER5_3 MATLAB code for KinetochoreSelectionTool_ver5_3.fig
%      KINETOCHORESELECTIONTOOL_VER5_3, by itself, creates a new KINETOCHORESELECTIONTOOL_VER5_3 or raises the existing
%      singleton*.
%
%      H = KINETOCHORESELECTIONTOOL_VER5_3 returns the handle to a new KINETOCHORESELECTIONTOOL_VER5_3 or the handle to
%      the existing singleton*.
%
%      KINETOCHORESELECTIONTOOL_VER5_3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KINETOCHORESELECTIONTOOL_VER5_3.M with the given input arguments.
%
%      KINETOCHORESELECTIONTOOL_VER5_3('Property','Value',...) creates a new KINETOCHORESELECTIONTOOL_VER5_3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KinetochoreSelectionTool_ver5_3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KinetochoreSelectionTool_ver5_3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help KinetochoreSelectionTool_ver5_3
% Last Modified by GUIDE v2.5 24-Jul-2015 17:05:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @KinetochoreSelectionTool_ver5_3_OpeningFcn, ...
    'gui_OutputFcn',  @KinetochoreSelectionTool_ver5_3_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function varargout = KinetochoreSelectionTool_ver5_3_OutputFcn(hObject, eventdata, handles)
...
% --- Executes just before KinetochoreSelectionTool_ver5_3 is made visible.
function KinetochoreSelectionTool_ver5_3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KinetochoreSelectionTool_ver5_3 (see VARARGIN)

% Choose default command line output for KinetochoreSelectionTool_ver5_3
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes KinetochoreSelectionTool_ver5_3 wait for user response (see UIRESUME)
...
%    N_frame = getappdata(hMainGui,'N_frame');
    ...
    for i = 1:Nframe
        ...
        %Add real time
        frame{i}.time_handle = text(0.1*img_size(2),0.1*img_size(1),[num2str(etime(timesdt(frame{i}.setting,'vector'),fr0time)),' sec'],'fontsize',12,...
            'Color','y','Clipping','on');
...
if isfield(handles,'location') == 0 | isempty(handles.location) 
    return
end
...
if eventData.Button == 3
    distance = sqrt((location(location(:,2,axesId)==framenum,4,axesId)-coord(1)).^2+(location(location(:,2,axesId)==framenum,5,axesId)-coord(2)).^2);
    kinids = location(location(:,2,axesId)==framenum,1,axesId);
    [~,ind] = min(distance);
...
switch button
    case 'Change ID'
        ...
        location(location(:,2,axesId)==framenum & location(:,1,axesId)==selected_kin,1,:) = newid;
        kinstate(kinstate(:,2)==framenum & kinstate(:,1)==selected_kin,1) = newid;
        kin{selected_kin}.frame(kin{selected_kin}.frame==framenum) = [];
        kin{selected_kin}.time(kin{selected_kin}.frame==framenum) = [];
        kin{selected_kin}.selected_pixel(:,:,kin{selected_kin}.frame==framenum) = [];
        frame{framenum}.kin_id(frame{framenum}.kin_id==selected_kin) = newid;
    case 'Delete'
        %set "accept or reject" parameter in kinstate matrix to 0 (which means rejected)
        kinstate(kinstate(:,1)==selected_kin & kinstate(:,2)==framenum,3) = 0;
        
        idx = find(frame{framenum}.kin_id == selected_kin);
        
        %hide rejected kinetochores in current frame
        set(frame{framenum}.kincircle_handle(idx,ishandle(frame{framenum}.kincircle_handle(idx,:))),'Visible','off')
        set(frame{framenum}.text_handle(idx,ishandle(frame{framenum}.text_handle(idx,:))),'Visible','off')
        
        %exclude rejected kinetochores out of selected pixels region
        selected_pixel = zeros(img_size(2),img_size(1));
        
        rem_kin = frame{framenum}.kin_id(frame{framenum}.kin_id~=selected_kin);
        for l = 1:length(rem_kin);
            selected_pixel = selected_pixel |...
                kin{rem_kin(l)}.selected_pixel(:,:,find(kin{rem_kin(l)}.frame==framenum));
        end
        frame{framenum}.selected_pixel = selected_pixel;
end

%check whether the selected kinetochore is not accepted in any frame
if sum(kinstate(kinstate(:,1)==selected_kin,3))==0
    kinlist = cellstr(get(handles.Kinetochore_listbox,'String'));
    kinlist(strcmp(kinlist,num2str(selected_kin))) = [];
    set(handles.Kinetochore_listbox,'Value',1);
    set(handles.Kinetochore_listbox,'String',kinlist)
end
...
% kinetochore structure (N kinetochoresXN frames cell matrix)
kin = cell(Nkin,1);
for i = 1:Nkin
    ind = res(:,8,1)==i;
    kin{i}.id = i;
