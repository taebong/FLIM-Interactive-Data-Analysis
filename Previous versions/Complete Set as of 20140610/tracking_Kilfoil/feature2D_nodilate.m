function r = feature2D_nodilate(img,lambda,w, masscut,Imin,field)
...
%           low mass 'noise' features. (Default is 0, i.e. accept all.)
...
%
%       7-29-03  Maria Kilfoil -- Matlab version
%       10-18-07 Vincent Pelletier -- added masscut to Matlab version
...
loc= localmax_nodilate(image,sep,field,Imin);
if (loc(1) == -1)
    r = -1;
    return
else
...
b = find( m > masscut); %only those features with a total mass higher than masscut are considered to be a real feature
nmax=length(b);
if nmax==0 
