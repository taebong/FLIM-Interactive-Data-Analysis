function [p,X2,sigma_p,sigma_y,corr,R_sq,cvg_hst, converged] = lm(func,p,time,data,weight,dp,p_min,p_max,c,fit_start,fit_end)

% [p,X2,sigma_p,sigma_y,corr,R_sq,cvg_hst] = lm(func,p,t,y_dat,weight,dp,p_min,p_max,c,fit_start,fit_end)
%
...
if nargin < 10, fit_start = 1; end
if nargin < 11, fit_end = length(data); end
...
[alpha,beta,X2,y_hat,dydp] = lm_matx(func,time,p,data,weight_sq,dp,c,fit_start,fit_end);
...
function dydp = lm_dydp(func,time,p,y,dp,c,fit_start,fit_end)
...
function [alpha,beta,Chi_sq,y_hat,dydp] = lm_matx(func,time,p,data,weight_sq,dp,c,fit_start,fit_end)
...
y_hat = feval(func,time,p,c);	% evaluate model using parameters 'p'
...
dydp = lm_dydp(func,time,p,y_hat,dp,c,fit_start,fit_end);
