function varargout = FittingGUI_ver3_2(varargin)
% FITTINGGUI_VER3_2 MATLAB code for FittingGUI_ver3_2.fig
%      FITTINGGUI_VER3_2, by itself, creates a new FITTINGGUI_VER3_2 or raises the existing
%      singleton*.
%
%      H = FITTINGGUI_VER3_2 returns the handle to a new FITTINGGUI_VER3_2 or the handle to
%      the existing singleton*.
%
%      FITTINGGUI_VER3_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FITTINGGUI_VER3_2.M with the given input arguments.
%
%      FITTINGGUI_VER3_2('Property','Value',...) creates a new FITTINGGUI_VER3_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FittingGUI_ver3_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FittingGUI_ver3_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FittingGUI_ver3_2

% Last Modified by GUIDE v2.5 03-Jul-2013 12:42:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FittingGUI_ver3_2_OpeningFcn, ...
    'gui_OutputFcn',  @FittingGUI_ver3_2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FittingGUI_ver3_2 is made visible.
function FittingGUI_ver3_2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FittingGUI_ver3_2 (see VARARGIN)

% Choose default command line output for FittingGUI_ver3_2
...
% UIWAIT makes FittingGUI_ver3_2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FittingGUI_ver3_2_OutputFcn(hObject, eventdata, handles)
...
if fitting_method == 1   % Do LS fitting
    for j = selected
        ...
        if nexpo == 1
            ...
            p_max = [10,1,5]';
        elseif nexpo == 2
            p_init = [0,1-est_noise/max(decay),3,0.5,1]';
            p_min = [-10,0.7,0.05,0,0.05]';
            p_max = [10,1,5,1,5]';
        else
...
elseif fitting_method == 3
    ...
    for j = selected
        ...
        if nexpo == 1
            ...
            dp = [1,0.01,0.01]';
        elseif nexpo == 2
            p_min = [-10,0.8,2.5,0.5,0.05]';
            p_max = [10,1,5,1,2.5]';
            dp = [1,0.01,0.01,0.01,0.01]';
        else
            errordlg('not available yet')
            return
        end
        ...
        tic;
        [p_fit,sigma_p,p,post,marg_post] = bayes_fit(@bayes_decay_model,time,decay,dp,p_min,p_max,nexpo,prior,fit_start,fit_end);
        toc;
...
tau1 = fit_result(3,1);
delta_tau1 = fit_result(3,2);
tau2 = fit_result(5,1);
delta_tau2 = fit_result(5,2);
