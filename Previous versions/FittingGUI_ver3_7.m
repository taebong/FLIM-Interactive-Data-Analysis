function varargout = FittingGUI_ver3_7(varargin)
% FITTINGGUI_VER3_7 MATLAB code for FittingGUI_ver3_7.fig
%      FITTINGGUI_VER3_7, by itself, creates a new FITTINGGUI_VER3_7 or raises the existing
%      singleton*.
%
%      H = FITTINGGUI_VER3_7 returns the handle to a new FITTINGGUI_VER3_7 or the handle to
%      the existing singleton*.
%
%      FITTINGGUI_VER3_7('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FITTINGGUI_VER3_7.M with the given input arguments.
%
%      FITTINGGUI_VER3_7('Property','Value',...) creates a new FITTINGGUI_VER3_7 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FittingGUI_ver3_7_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FittingGUI_ver3_7_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FittingGUI_ver3_7

% Last Modified by GUIDE v2.5 05-Aug-2014 09:49:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FittingGUI_ver3_7_OpeningFcn, ...
    'gui_OutputFcn',  @FittingGUI_ver3_7_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FittingGUI_ver3_7 is made visible.
function FittingGUI_ver3_7_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FittingGUI_ver3_7 (see VARARGIN)

% Choose default command line output for FittingGUI_ver3_7
...
decaytoshow = handles.decay_struct{1}.decay;
timetoshow = handles.decay_struct{1}.time;
handles.decay_struct{1}.decay_handle = ...
    semilogy(timetoshow,decaytoshow,'ro');
%xlim([0,10])
...
fit_start_x = timetoshow(min(find(decaytoshow>0)));
fit_end_x = timetoshow(max(find(decaytoshow>0)));
noise_region_from = timetoshow(find(decaytoshow == max(decaytoshow)))-1;
noise_region_to = timetoshow(find(decaytoshow == max(decaytoshow)))-0.5;
...
% UIWAIT makes FittingGUI_ver3_7 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FittingGUI_ver3_7_OutputFcn(hObject, eventdata, handles)
...
for i = selected
   handles.decay_struct{i}.fit_result(eventdata.Indices) = eventdata.NewData;
end
guidata(hObject,handles)
