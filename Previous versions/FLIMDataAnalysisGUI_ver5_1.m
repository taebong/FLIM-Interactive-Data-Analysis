...
function varargout = FLIMDataAnalysisGUI_ver5_1(varargin)
% FLIMDATAANALYSISGUI_VER5_1 MATLAB code for FLIMDataAnalysisGUI_ver5_1.fig
%      FLIMDATAANALYSISGUI_VER5_1, by itself, creates a new FLIMDATAANALYSISGUI_VER5_1 or raises the existing
%      singleton*.
%
%      H = FLIMDATAANALYSISGUI_VER5_1 returns the handle to a new FLIMDATAANALYSISGUI_VER5_1 or the handle to
%      the existing singleton*.
%
%      FLIMDATAANALYSISGUI_VER5_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FLIMDATAANALYSISGUI_VER5_1.M with the given input arguments.
%
%      FLIMDATAANALYSISGUI_VER5_1('Property','Value',...) creates a new FLIMDATAANALYSISGUI_VER5_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FLIMDataAnalysisGUI_ver5_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FLIMDataAnalysisGUI_ver5_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FLIMDataAnalysisGUI_ver5_1

% Last Modified by GUIDE v2.5 04-Sep-2013 23:20:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FLIMDataAnalysisGUI_ver5_1_OpeningFcn, ...
    'gui_OutputFcn',  @FLIMDataAnalysisGUI_ver5_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FLIMDataAnalysisGUI_ver5_1 is made visible.
function FLIMDataAnalysisGUI_ver5_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FLIMDataAnalysisGUI_ver5_1 (see VARARGIN)

% Choose default command line output for FLIMDataAnalysisGUI_ver5_1
...
% so window can get raised using FLIMDataAnalysisGUI_ver5_1.
if strcmp(get(hObject,'Visible'),'off')
    
end

% UIWAIT makes FLIMDataAnalysisGUI_ver5_1 wait for user response (see UIRESUME)
% uiwait(handles.MainGui);


% --- Outputs from this function are returned to the command line.
function varargout = FLIMDataAnalysisGUI_ver5_1_OutputFcn(hObject, eventdata, handles)
...
num_lines = 1;
def = {eg_fname,['1:',num2str(eg_Nframe)],'4',num2str(eg_repeat)...
    ,'60','32','avi',''};
...
for i = framenum
    ...
    writeVideo(writerObj,imadjust(img));
...
reshapedselectedpix = repmat(reshape(selected_pixel'-prev_selected_pixel',[1,size(selected_pixel')]),[length(decay),1,1]);
...
figname = 'KinetochoreSelectionTool_ver2_2';
