function varargout = KinetochoreSelectionTool_ver2_2(varargin)
% KINETOCHORESELECTIONTOOL_VER2_2 MATLAB code for KinetochoreSelectionTool_ver2_2.fig
%      KINETOCHORESELECTIONTOOL_VER2_2, by itself, creates a new KINETOCHORESELECTIONTOOL_VER2_2 or raises the existing
%      singleton*.
%
%      H = KINETOCHORESELECTIONTOOL_VER2_2 returns the handle to a new KINETOCHORESELECTIONTOOL_VER2_2 or the handle to
%      the existing singleton*.
%
%      KINETOCHORESELECTIONTOOL_VER2_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KINETOCHORESELECTIONTOOL_VER2_2.M with the given input arguments.
%
%      KINETOCHORESELECTIONTOOL_VER2_2('Property','Value',...) creates a new KINETOCHORESELECTIONTOOL_VER2_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KinetochoreSelectionTool_ver2_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KinetochoreSelectionTool_ver2_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help KinetochoreSelectionTool_ver2_2

% Last Modified by GUIDE v2.5 08-Sep-2013 17:46:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @KinetochoreSelectionTool_ver2_2_OpeningFcn, ...
    'gui_OutputFcn',  @KinetochoreSelectionTool_ver2_2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before KinetochoreSelectionTool_ver2_2 is made visible.
function KinetochoreSelectionTool_ver2_2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KinetochoreSelectionTool_ver2_2 (see VARARGIN)

% Choose default command line output for KinetochoreSelectionTool_ver2_2
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes KinetochoreSelectionTool_ver2_2 wait for user response (see UIRESUME)
...
function varargout = KinetochoreSelectionTool_ver2_2_OutputFcn(hObject, eventdata, handles)
...
for i = 1:length(res(:,1))
    ...
    kin{id}.selected_pixel(:,:,idx) = ((X-cen_x).^2+(Y-cen_y).^2<=R_cover^2);
...
koi = unique(koi);  %kinetoc    hore ids of kinetochores that will be analyzed
...
if name ~= 0
    save([pathname,name],'kin','kinpair','bayessetting','frame','pixsize','repeat','spaxis');
end
