function varargout = KinetochoreSelectionTool_ver4_1(varargin)
% KINETOCHORESELECTIONTOOL_VER4_1 MATLAB code for KinetochoreSelectionTool_ver4_1.fig
%      KINETOCHORESELECTIONTOOL_VER4_1, by itself, creates a new KINETOCHORESELECTIONTOOL_VER4_1 or raises the existing
%      singleton*.
%
%      H = KINETOCHORESELECTIONTOOL_VER4_1 returns the handle to a new KINETOCHORESELECTIONTOOL_VER4_1 or the handle to
%      the existing singleton*.
%
%      KINETOCHORESELECTIONTOOL_VER4_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KINETOCHORESELECTIONTOOL_VER4_1.M with the given input arguments.
%
%      KINETOCHORESELECTIONTOOL_VER4_1('Property','Value',...) creates a new KINETOCHORESELECTIONTOOL_VER4_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KinetochoreSelectionTool_ver4_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KinetochoreSelectionTool_ver4_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help KinetochoreSelectionTool_ver4_1
% Last Modified by GUIDE v2.5 03-May-2014 15:43:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @KinetochoreSelectionTool_ver4_1_OpeningFcn, ...
    'gui_OutputFcn',  @KinetochoreSelectionTool_ver4_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function varargout = KinetochoreSelectionTool_ver4_1_OutputFcn(hObject, eventdata, handles)
...
% --- Executes just before KinetochoreSelectionTool_ver4_1 is made visible.
function KinetochoreSelectionTool_ver4_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KinetochoreSelectionTool_ver4_1 (see VARARGIN)

% Choose default command line output for KinetochoreSelectionTool_ver4_1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes KinetochoreSelectionTool_ver4_1 wait for user response (see UIRESUME)
...
for i = 1:length(frame{selected}.kin_id)
    kintoshow(i,1) = kinstate(kinstate(:,1)==frame{selected}.kin_id(i) & kinstate(:,2)==selected,13)==1;
...
set(frame{preselected}.image2_handle,'Visible','off')
set(frame{selected}.image2_handle,'Visible','on');

%toc

if frame{preselected}.N_kin > 0
...
%col5 and 6: intrakinetochore vector, ch2 position-ch1 position. (-99,-99) if either is
%missing.
kinstate(:,5:6) = (res(:,1:2,1).*res(:,1:2,2)~=0).*(res(:,1:2,2)-res(:,1:2,1)) + ...
    (res(:,1:2,1).*res(:,1:2,2)==0).*-99;
%col7 and 8: velocity, (-99,-99) if N/A, in unit of pix/frame
Nkin = max(res(:,8,1));
for i = 1:Nkin
    idx = find(res(:,8,1)==i);
    if length(idx) < 2
        ...
        kinstate(idx(1:end-1),7:8) = ...
            ((kinstate(idx(2:end),3:4)-kinstate(idx(1:end-1),3:4)))./repmat((kinstate(idx(2:end),2)-kinstate(idx(1:end-1),2)),1,2);
        kinstate(idx(end),7:8) = -99;
    end
end
%col9 and 10: Rg^2 in ch1 and ch2
kinstate(:,9:10) = squeeze(res(:,4,:));
%col11: 1 if leading, -1 if trailing, 0 if NA
kinstate(:,11) = (sum(kinstate(:,5:6).*kinstate(:,7:8),2)>0 & prod(kinstate(:,5:8)>-99,2)>0)*1+...
    (sum(kinstate(:,5:6).*kinstate(:,7:8),2)<0 & prod(kinstate(:,5:8)>-99,2)>0)*(-1);
...
kinstate(:,13) = 1;
%col14: 1 if there is another kinetochore within the distance defined by
%selection coverage (e.g. Rg), 0 if not.
kinstate(:,14) = 0;
for fr = 1:N_frame
    %kinetochores in the given frame
    kininfr = frame{fr}.kin_id;
    
    for k = 1:length(kininfr)
        kinkstate = kinstate(kinstate(:,1)==kininfr(k) & kinstate(:,2)==fr,:);
        if kinkstate(14)
            continue;
        else
            for l = 1:length(kininfr)
                if k == l
                    continue;
                end
                kinlstate = kinstate(kinstate(:,1)==kininfr(l) & kinstate(:,2)==fr,:);
                pairdist = sqrt(sum((kinkstate(3:4)-kinlstate(3:4)).^2));
                if pairdist < coverage*sqrt((max(kinkstate(8:9))+max(kinlstate(8:9))));
                    kinstate(kinstate(:,1)==kininfr(k) & kinstate(:,2)==fr,14)=1;
                    kinstate(kinstate(:,1)==kininfr(l) & kinstate(:,2)==fr,14)=1;
                end
            end
        end
    end
...
kinstateGauss = zeros(size(kinstate,1),
...
for fr = 1:Nframe
    ...
    for k = 1:length(kininfr)
        ...
        %if the kinetochore has an overlap with another kinetochore,
        %fit joint guassian distribution
        if kinkstate(14) == 1
            ...
            %construct kinstateGauss matrix using resGauss.
            %col3 and 4: kinetochore position. mean if both channel exist, take one value
            %otherwises.
            kinstate(kinstateind,3:4) = ((resGauss(resind,1:2,1)>0).*resGauss(:,1:2,1)+(resGauss(resind,1:2,2)>0).*res(:,1:2,2))./...
                ((res(:,1:2,1)>0)+(res(:,1:2,2)>0));
            kinstate(kinstateind,3:4) = 
        end
    end    
end
...
for k = 1:N_rm
    ...
    for j = 1:length(frame_num)
        ...
        kinstate(kinstate(:,1)==kin_selected(k) & kinstate(:,2)==frame_num(j),13)=0;
