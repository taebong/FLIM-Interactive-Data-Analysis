function varargout = KinetochoreSelectionTool_ver4_4(varargin)
% KINETOCHORESELECTIONTOOL_VER4_4 MATLAB code for KinetochoreSelectionTool_ver4_4.fig
%      KINETOCHORESELECTIONTOOL_VER4_4, by itself, creates a new KINETOCHORESELECTIONTOOL_VER4_4 or raises the existing
%      singleton*.
%
%      H = KINETOCHORESELECTIONTOOL_VER4_4 returns the handle to a new KINETOCHORESELECTIONTOOL_VER4_4 or the handle to
%      the existing singleton*.
%
%      KINETOCHORESELECTIONTOOL_VER4_4('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in KINETOCHORESELECTIONTOOL_VER4_4.M with the given input arguments.
%
%      KINETOCHORESELECTIONTOOL_VER4_4('Property','Value',...) creates a new KINETOCHORESELECTIONTOOL_VER4_4 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before KinetochoreSelectionTool_ver4_4_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to KinetochoreSelectionTool_ver4_4_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help KinetochoreSelectionTool_ver4_4
% Last Modified by GUIDE v2.5 10-May-2014 18:07:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @KinetochoreSelectionTool_ver4_4_OpeningFcn, ...
    'gui_OutputFcn',  @KinetochoreSelectionTool_ver4_4_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function varargout = KinetochoreSelectionTool_ver4_4_OutputFcn(hObject, eventdata, handles)
...
% --- Executes just before KinetochoreSelectionTool_ver4_4 is made visible.
function KinetochoreSelectionTool_ver4_4_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to KinetochoreSelectionTool_ver4_4 (see VARARGIN)

% Choose default command line output for KinetochoreSelectionTool_ver4_4
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes KinetochoreSelectionTool_ver4_4 wait for user response (see UIRESUME)
...
vardistcutoff = str2double(get(handles.VarDistCutoff_eidt,'String'));
        ...
        if pass
            ...
            for k = 1:length(frameid)
                ...
                if stretch(stretchindi,3)~=-99
                    innerprod = innerprod + sum(disp(k,:).*stretch(stretchindi,3:4))/dist(k);
                    ninnerprod = ninnerprod+1;
                end
                if stretch(stretchindj,3)~=-99
                    innerprod = innerprod - sum(disp(k,:).*stretch(stretchindj,3:4))/dist(k);
                    ninnerprod = ninnerprod+1;
                end
            end
...
for k = 1:length(frameid)
    ...
    if stretch(stretchind1,3)~=-99
        innerprod = innerprod + sum(disp(k,:).*stretch(stretchind1,3:4))/dist(k);
        ninnerprod = ninnerprod+1;
    end
    if stretch(stretchind2,3)~=-99
        innerprod = innerprod - sum(disp(k,:).*stretch(stretchind2,3:4))/dist(k);
        ninnerprod = ninnerprod+1;
    end
end
innerprod = innerprod/ninnerprod;
...
disp(['ID Frame kin1 kin2 interkinetochore dist'])
