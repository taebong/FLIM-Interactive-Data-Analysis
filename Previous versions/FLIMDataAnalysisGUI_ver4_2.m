function varargout = FLIMDataAnalysisGUI_ver4_2(varargin)
% FLIMDATAANALYSISGUI_VER4_2 MATLAB code for FLIMDataAnalysisGUI_ver4_2.fig
%      FLIMDATAANALYSISGUI_VER4_2, by itself, creates a new FLIMDATAANALYSISGUI_VER4_2 or raises the existing
%      singleton*.
%
%      H = FLIMDATAANALYSISGUI_VER4_2 returns the handle to a new FLIMDATAANALYSISGUI_VER4_2 or the handle to
%      the existing singleton*.
%
%      FLIMDATAANALYSISGUI_VER4_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FLIMDATAANALYSISGUI_VER4_2.M with the given input arguments.
%
%      FLIMDATAANALYSISGUI_VER4_2('Property','Value',...) creates a new FLIMDATAANALYSISGUI_VER4_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FLIMDataAnalysisGUI_ver4_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FLIMDataAnalysisGUI_ver4_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FLIMDataAnalysisGUI_ver4_2

% Last Modified by GUIDE v2.5 04-Mar-2013 23:25:07

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FLIMDataAnalysisGUI_ver4_2_OpeningFcn, ...
    'gui_OutputFcn',  @FLIMDataAnalysisGUI_ver4_2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FLIMDataAnalysisGUI_ver4_2 is made visible.
function FLIMDataAnalysisGUI_ver4_2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FLIMDataAnalysisGUI_ver4_2 (see VARARGIN)

% Choose default command line output for FLIMDataAnalysisGUI_ver4_2
...
% so window can get raised using FLIMDataAnalysisGUI_ver4_2.
if strcmp(get(hObject,'Visible'),'off')
    
end


% UIWAIT makes FLIMDataAnalysisGUI_ver4_2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FLIMDataAnalysisGUI_ver4_2_OutputFcn(hObject, eventdata, handles)
...
switch NumOfImages
    ...
    case 1
        axes(handles.Image_axes)
        if ishandle(image_struct{selected}.image_handle)
            delete(image_struct{selected}.image_handle);
        end
        if ishandle(image_struct{selected}.selected_pixel_handle)
            delete(image_struct{selected}.selected_pixel_handle);
        end
        
        axes(handles.Decay_axes)
        if ishandle(image_struct{selected}.decay_handle)
...
x = 1; y = 1;
while button==1 && x<=XL(2) && x>=XL(1) && y<=YL(2) && y>=XL(1)
    [x,y,button]=ginput(1);
    x = round(x); y = round(y);
...
delete(selected_pixel_handle);
handles.image_struct{selected}.selected_pixel = zeros(size(selected_pixel));
handles.image_struct{selected}.selected_pixel_handle = [];
handles.image_struct{selected}.decay = zeros(size(decay));
...
% --- Executes on button press in SelectAbove_pushbutton.
function SelectAbove_pushbutton_Callback(hObject, eventdata, handles)
% hObject    handle to SelectAbove_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
[y,x] = find(squeeze(sum(flim,1))>above_number);
...
decay_sel = get(handles.SavedDecay_listbox,'Value');
dt = str2double(get(handles.dt_edit,'String'));
loaded_irf = load('IRF.mat');
irf = loaded_irf.decay;
saved_decay = handles.saved_decay(decay_sel);

save('decay_to_fit.mat','dt','irf','saved_decay');

currentFolder = pwd;
run([currentFolder '/FittingGUI_ver1_1.m'])
...
save('decay_to_phasor.mat','dt','freq','ref','ref_tau','saved_decay');
