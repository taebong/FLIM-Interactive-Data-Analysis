...
function varargout = FLIMDataAnalysisGUI_ver7_2(varargin)
% FLIMDATAANALYSISGUI_VER7_2 MATLAB code for FLIMDataAnalysisGUI_ver7_2.fig
%      FLIMDATAANALYSISGUI_VER7_2, by itself, creates a new FLIMDATAANALYSISGUI_VER7_2 or raises the existing
%      singleton*.
%d
%      H = FLIMDATAANALYSISGUI_VER7_2 returns the handle to a new FLIMDATAANALYSISGUI_VER7_2 or the handle to
%      the existing singleton*.
%
%      FLIMDATAANALYSISGUI_VER7_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FLIMDATAANALYSISGUI_VER7_2.M with the given input arguments.
%
%      FLIMDATAANALYSISGUI_VER7_2('Property','Value',...) creates a new FLIMDATAANALYSISGUI_VER7_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FLIMDataAnalysisGUI_ver7_2_OpeningFcn gets called.  An
...
% Edit the above text to modify the response to help FLIMDataAnalysisGUI_ver7_2

% Last Modified by GUIDE v2.5 12-Feb-2015 13:52:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FLIMDataAnalysisGUI_ver7_2_OpeningFcn, ...
    'gui_OutputFcn',  @FLIMDataAnalysisGUI_ver7_2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FLIMDataAnalysisGUI_ver7_2 is made visible.
function FLIMDataAnalysisGUI_ver7_2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FLIMDataAnalysisGUI_ver7_2 (see VARARGIN)

% Choose default command line output for FLIMDataAnalysisGUI_ver7_2
...
% so window can get raised using FLIMDataAnalysisGUI_ver7_2.
if strcmp(get(hObject,'Visible'),'off')
    
end

% UIWAIT makes FLIMDataAnalysisGUI_ver7_2 wait for user response (see UIRESUME)
% uiwait(handles.MainGui);


% --- Outputs from this function are returned to the command line.
function varargout = FLIMDataAnalysisGUI_ver7_2_OutputFcn(hObject, eventdata, handles)
    ...
    for i = 1:Nframe
        ...
        if showM1 && showM2
            ...
            if i == 1
                ...
                imwrite(img1,fullfile(pathname,[filename(1:end-4),'_m1',filename(end-3:end)]),'writemode','append');
                imwrite(img2,fullfile(pathname,[filename(1:end-4),'_m2',filename(end-3:end)]),'writemode','append');
            end
        else
            if i == 1
...
%reshapedselectedpix = repmat(reshape(selected_pixel',[1,size(selected_pixel')]),[length(decay),1,1]);
[is,js] = find(selected_pixel);
decay = zeros(size(decay));
ioi = is>=x_min & is<=x_max & js>=y_min & js <=y_max;
is = is(ioi);
js = js(ioi);
for ind = 1:length(is)
...
%        reshapedselectedpix = repmat(reshape(selected_pixel',[1,size(selected_pixel')]),[length(decay_data),1,1]);
        ...
        [is,js] = find(selected_pixel);
        decay_data = zeros(size(decay_data));
        ioi = is>=x_min & is<=x_max & js>=y_min & js <=y_max;
        is = is(ioi);
        js = js(ioi);
        for ind2 = 1:length(is)
            decay_data = decay_data+flim(:,js(ind2),is(ind2));
        end
...
for idx = 1:length(selected);
    i = selected(idx);
    %time axis
    dt = image_struct{i}.dt;
    
    active_region = image_struct{i}.active_region;
    img = image_struct{i}.image;
    decay = image_struct{i}.decay;
    decay_handle = image_struct{i}.decay_handle;
    time = (1:length(decay))'*dt;
    flim = image_struct{i}.flim;
    selected_pixel_handle = image_struct{i}.selected_pixel_handle;
    ...
    %update image_struct
    image_struct{i}.decay = decay;
    image_struct{i}.decay_handle = decay_handle;
    image_struct{i}.selected_pixel = selected_pixel;
    image_struct{i}.selected_pixel_handle = selected_pixel_handle;    
end
...
figname = 'KinetochoreSelectionTool_ver5_2';
