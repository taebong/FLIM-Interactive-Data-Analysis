function [p,X2,sigma_p,sigma_y,corr,R_sq,cvg_hst, converged] = lm(func,p,time,data,time_irf,irf,weight,dp,p_min,p_max,c,fit_start,fit_end)
...
[alpha,beta,X2,y_hat,dydp] = lm_matx(func,time,p,data,time_irf,irf,weight_sq,dp,c,fit_start,fit_end);
...
while ( ~stop & iteration <= MaxIter )		% --- Main Loop
    ...
    y1 = feval(func,time,p_try,c,time_irf,irf);
    ...
    if ( Update_Type == 2 )
        ...
        y2 = feval(func,time,p_try,c,time_irf,irf);
...
function dydp = lm_dydp(func,time,p,y,time_irf,irf,dp,c,fit_start,fit_end)
...
for j=1:n                       % loop over all parameters
    ...
    if del(j) ~= 0
        y1=feval(func,time,p,c,time_irf,irf);
        y1=y1(fit_start:fit_end);
        func_calls = func_calls + 1;
        ...
        if (dp(j) < 0)		% backwards difference
            ...
            p(j) = ps(j) - del(j);
            y2 = feval(func,time,p,c,time_irf,irf);
            y2 = y2(fit_start:fit_end);
...
function [alpha,beta,Chi_sq,y_hat,dydp] = lm_matx(func,time,p,data,time_irf,irf,weight_sq,dp,c,fit_start,fit_end)
...
y_hat = feval(func,time,p,c,time_irf,irf);	% evaluate model using parameters 'p'
...
dydp = lm_dydp(func,time,p,y_hat,time_irf,irf,dp,c,fit_start,fit_end);
