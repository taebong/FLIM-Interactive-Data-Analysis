function [lub] = trackmemkin(xyzs,maxdisp,dim,goodenough,memory)

%      Written 02-2005 by Naama Gal and Maria Kilfoil
%       Modified 03-2014 by Tae Yeon Yoo for dual color kinetochore tracking 
...
% Tae:
% first two dimensions are x and y, and the third is angle of
% intrakinetochore vector from +x-axis. The measure between (x1,y1,theta1)
% and (x2,y2,theta2) is defined to be:
% (x1-x2)^2+(y1-y2)^2+meas(theta1-theta2) where
% meas(theta1-theta2) = 0 if cos(theta1-theta2)>0 or either/both theta1
% or/and theta2 = -10  (which means the data is missing)
% and = maxdisp^2 if cos(theta1-theta2)<0
...
z = length(w);
if z==0
    warning('ERROR - All positions are at the same time!')
else
    if sum(st(w)-st(w(1)))~=0
        %warning('WARNING - Time vector gapped or not evenly gridded!')
    end
end
    ...
    if m>0  
        ...
        if notnsqrd
            ...
            for j=1:n
                ...
                if ngood ~= 0
                    ...
                    for d=1:dim
                        ...
                        elseif d == 3
                            distq = distq + (xyi(map,d)*pos(j,d)~=0 & cos(xyi(map,d)-pos(j,d))<0)*maxdisp^2;  
                        end
                    end
                    ltmax = distq < maxdisq;
                    rowtot(j) = sum( ltmax );
            ...
            for d=1:dim
                ...
                elseif d == 3
                    dq = dq+ (x(xmat).*y(ymat)~=0 & cos(x(xmat)-(y(ymat)))<0)*maxdisp^2;
                end
            end
            ltmax = ( dq < maxdisq );
        ...
        if nontrivial
            ...
            for j=1:ydim
                ...
                for d=1:dim
                    ...
                    elseif d == 3
                        distq = distq + (xyi(labelx,d)*pos(labely(j),d)~=0 & cos(xyi(labelx,d)-pos(labely(j),d))<0)'*maxdisp^2;
                    end
                end
