function varargout = FLIMDataAnalysisGUI_ver4_7(varargin)
% FLIMDATAANALYSISGUI_VER4_7 MATLAB code for FLIMDataAnalysisGUI_ver4_7.fig
%      FLIMDATAANALYSISGUI_VER4_7, by itself, creates a new FLIMDATAANALYSISGUI_VER4_7 or raises the existing
%      singleton*.
%
%      H = FLIMDATAANALYSISGUI_VER4_7 returns the handle to a new FLIMDATAANALYSISGUI_VER4_7 or the handle to
%      the existing singleton*.
%
%      FLIMDATAANALYSISGUI_VER4_7('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FLIMDATAANALYSISGUI_VER4_7.M with the given input arguments.
%
%      FLIMDATAANALYSISGUI_VER4_7('Property','Value',...) creates a new FLIMDATAANALYSISGUI_VER4_7 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FLIMDataAnalysisGUI_ver4_7_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FLIMDataAnalysisGUI_ver4_7_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FLIMDataAnalysisGUI_ver4_7

% Last Modified by GUIDE v2.5 19-Jul-2013 12:58:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FLIMDataAnalysisGUI_ver4_7_OpeningFcn, ...
    'gui_OutputFcn',  @FLIMDataAnalysisGUI_ver4_7_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FLIMDataAnalysisGUI_ver4_7 is made visible.
function FLIMDataAnalysisGUI_ver4_7_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FLIMDataAnalysisGUI_ver4_7 (see VARARGIN)

% Choose default command line output for FLIMDataAnalysisGUI_ver4_7
...
% so window can get raised using FLIMDataAnalysisGUI_ver4_7.
if strcmp(get(hObject,'Visible'),'off')
    
end

% UIWAIT makes FLIMDataAnalysisGUI_ver4_7 wait for user response (see UIRESUME)
% uiwait(handles.MainGui);


% --- Outputs from this function are returned to the command line.
function varargout = FLIMDataAnalysisGUI_ver4_7_OutputFcn(hObject, eventdata, handles)
...
%         end
        ...
        image_struct{i}.selected_pixel = zeros(size(img));
    ...
    if NumOfPrevImages == 0
        ...
        set(handles.FileName_listbox,'String',filename);
    else
        ...
        set(handles.FileName_listbox,'String',[names;filename']);
        set(handles.FileName_listbox,'Value',NumOfPrevImages+1);
    end
    ...
    Switch_pushbuttons(handles,'on','image')
...
tempind = find(findstr(eg_fname,'sec')>ind);
ind2 = tempind(1);
...
num_lines = 1;
def = {eg_fname,['1:',num2str(eg_Nframe)],'2',num2str(eg_repeat)...
    ,'60','32','avi',''};
...
for i = framenum
    ...
    text(0.9*size(img,2)-round(barsize/pixsize),0.9*size(img,1)-5,...
        [num2str(barsize), '{\mu}', 'm'],'Color','yellow','fontsize',12)
    %Add real time
    text(0.1*size(img,2),0.1*size(img,1),[num2str((i-1)*repeat),' sec'],'fontsize',12,...
    'Color','y')
...
movie2avi(m,[pathname,filename],'fps',framerate);
...
while button==1
    ...
    if button ==1 && x>=x_min && x<=x_max && y>=y_min && y<=y_max
        ...
        selected_pixel_handle = plot(xx,yy,'ks','MarkerFaceColor','k','MarkerSize',5); %mark the point
...
if isempty(active_region)
    ...
    for i = 1:length(decay)
        decay(i) = decay(i) - sum(sum(squeeze(flim(i,y_min:y_max,x_min:x_max)).*selected_pixel(x_min:x_max,y_min:y_max)'));
    end
...
[y,x] = find(squeeze(sum(flim,1))>above_number & squeeze(sum(flim,1))<below_number);
prev_selected_pixel = selected_pixel;

axes(handles.Image_axes);

for i = 1:length(x)
    if x(i)>=x_min && x(i)<=x_max && y(i)>=y_min && y(i)<=y_max
        selected_pixel(x(i),y(i)) = 1;
        decay = decay + (selected_pixel(x(i),y(i))-prev_selected_pixel(x(i),y(i)))*flim(:,y(i),x(i));
    end
end
...
if button == 1
    ...
    for i = 1:length(time);
        decay(i) = sum(sum(squeeze(flim(i,y_min:y_max,x_min:x_max)).*selected_pixel(x_min:x_max,y_min:y_max)'));
    end
...
for i = selected(selected~=current)
    ...
    axes(handles.Image_axes);
    selected_pixel_handle = plot(xx,yy,'ks','MarkerFaceColor','k','MarkerSize',5);
...
figname = 'KinetochoreSelectionTool_ver1_2';
