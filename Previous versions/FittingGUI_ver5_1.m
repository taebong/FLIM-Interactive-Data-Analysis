function varargout = FittingGUI_ver5_1(varargin)
% FITTINGGUI_VER5_1 MATLAB code for FittingGUI_ver5_1.fig
%      FITTINGGUI_VER5_1, by itself, creates a new FITTINGGUI_VER5_1 or raises the existing
%      singleton*.
%
%      H = FITTINGGUI_VER5_1 returns the handle to a new FITTINGGUI_VER5_1 or the handle to
%      the existing singleton*.
%
%      FITTINGGUI_VER5_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FITTINGGUI_VER5_1.M with the given input arguments.
%
%      FITTINGGUI_VER5_1('Property','Value',...) creates a new FITTINGGUI_VER5_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FittingGUI_ver5_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FittingGUI_ver5_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FittingGUI_ver5_1

% Last Modified by GUIDE v2.5 23-Nov-2014 22:46:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FittingGUI_ver5_1_OpeningFcn, ...
    'gui_OutputFcn',  @FittingGUI_ver5_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FittingGUI_ver5_1 is made visible.
function FittingGUI_ver5_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FittingGUI_ver5_1 (see VARARGIN)

% Choose default command line output for FittingGUI_ver5_1
...
% UIWAIT makes FittingGUI_ver5_1 wait for user response (see UIRESUME)
% uiwait(handles.FittingGui);


% --- Outputs from this function are returned to the command line.
function varargout = FittingGUI_ver5_1_OutputFcn(hObject, eventdata, handles)
...
% --- Executes on button press in GroupBasedOnTime_pushbutton.
function GroupBasedOnTime_pushbutton_Callback(hObject, eventdata, handles)
% hObject    handle to GroupBasedOnTime_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
decay_struct = handles.decay_struct;
selected = get(handles.Decay_listbox,'Value');
...
% --- Executes on button press in PlotFitResult_pushbutton.
function PlotFitResult_pushbutton_Callback(hObject, eventdata, handles)
% hObject    handle to PlotFitResult_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
fit_start_x = currenttime(min(find(currentdecay>0)));
fit_end_x = currenttime(max(find(currentdecay>0)));
...
for j = selected
    decay_struct{j} = DecayFLIMFit(decay_struct{j},decay_struct{j}.fit_result,...
    ...
    elseif fitting_method == 2
        ...
        if handles.Mode_radiobutton.Value == 1
            for i = 1:(nexpo*2+1)
                [f,xi] = ksdensity(psampled(i,pmin(i):dp(i):pmax(i)));
                [~,ind] = max(f);
                pfit(i) = xi(ind);
            end
        end
...
function WithinTime_edit_Callback(hObject, eventdata, handles)
% hObject    handle to WithinTime_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of WithinTime_edit as text
%        str2double(get(hObject,'String')) returns contents of WithinTime_edit as a double


% --- Executes during object creation, after setting all properties.
function WithinTime_edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to WithinTime_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
for i = 1:Nselected
   decay_struct = decay_structs{selected(i)};
   fit_result = decay_struct.fit_result;
   
   param(:,i) = fit_result(pind,1);
   dparam(:,i) = fit_result(pind,2);
   
   if decay_struct.fitting_method == 2
       psampled = decay_struct.post;
       pmode(:,i) = mode(psampled,2);
       pquant_up(:,i) = quantile(psampled,0.5+alpha/2,2);
       pquant_down(:,i) = quantile(psampled,0.5-alpha/2,2);
   elseif decay_struct.fitting_method == 3
       marg_post = decay_struct.marg_post;
       p_vec = decay_struct.p_vec;
       for j = 1:Nshow
           [~,maxind] = max(marg_post{pind(j)});
           pmode(j,i) = p_vec{pind(j)}(maxind);
           
           cdf = cumsum(marg_post{pind(j)});
           [~,ind_up] = min(abs(cdf-(0.5+alpha/2)));
           [~,ind_down] = min(abs(cdf-(0.5-alpha/2)));
           pquant_up(j,i) = p_vec{pind(j)}(ind_up);
           pquant_down(j,i) = p_vec{pind(j)}(ind_down);
       end
   end
...
str = hObject.String;
if handles.FittingMethod_popupmenu.Value == 2
    if strcmp(str,'Mean')
        
    elseif strcmp(str,'Mode')
    end
elseif handles.FittingMethod_popupmenu.Value == 3
    if strcmp(str,'Mean')
...
output_decay_struct.pmode = [pmode,pquant_down,pquant_up];