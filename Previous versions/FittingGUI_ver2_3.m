function varargout = FittingGUI_ver2_3(varargin)
% FITTINGGUI_VER2_3 MATLAB code for FittingGUI_ver2_3.fig
%      FITTINGGUI_VER2_3, by itself, creates a new FITTINGGUI_VER2_3 or raises the existing
%      singleton*.
%
%      H = FITTINGGUI_VER2_3 returns the handle to a new FITTINGGUI_VER2_3 or the handle to
%      the existing singleton*.
%
%      FITTINGGUI_VER2_3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FITTINGGUI_VER2_3.M with the given input arguments.
%
%      FITTINGGUI_VER2_3('Property','Value',...) creates a new FITTINGGUI_VER2_3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FittingGUI_ver2_3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FittingGUI_ver2_3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FittingGUI_ver2_3

% Last Modified by GUIDE v2.5 12-Jun-2013 21:49:07

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FittingGUI_ver2_3_OpeningFcn, ...
    'gui_OutputFcn',  @FittingGUI_ver2_3_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FittingGUI_ver2_3 is made visible.
function FittingGUI_ver2_3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FittingGUI_ver2_3 (see VARARGIN)

% Choose default command line output for FittingGUI_ver2_3
...
for i = 1:NumOfDecays
   ...
   %fit_result = zeros(8,3);
   %fit_result(1,3) = 
   handles.decay_struct{i}.fit_result = zeros(8,3);
...
set(handles.FitResult_table,'Data',zeros(8,3));

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes FittingGUI_ver2_3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FittingGUI_ver2_3_OutputFcn(hObject, eventdata, handles)
...
tau2 = fit_result(6,1);
if A1>0
    set(handles.DecayRatio_text,'String',[num2str(A1/(A1+A2),2), ' : ', num2str(A2/(A1+A2),2)])
else
    set(handles.DecayRatio_text,'String',' ')
end
if tau1>0
...
grouped_decay.fit_result = zeros(8,3);
...
%elseif fitting_method == 1   % Do LS fitting
    ...
    for j = selected
        ...
        if nexpo == 1
            p_init(2:4) = [max(decay),3,est_noise]';
            p_min(2:4) = [0.5*max(decay),0.05,0.01]';
            p_max(2:4) = [1.2*max(decay),5,est_noise+10]';
        elseif nexpo == 2
            p_init(2:6) = [0.5*max(decay),3,est_noise,0.5*max(decay),0.5]';
            p_min(2:6) = [0.2*max(decay),0.05,0.01,1,0.05]';
            p_max(2:6) = [1.1*max(decay),5,est_noise+10,1.1*max(decay),5]';
        elseif nexpo == 3
            p_init(2:8) = [0.6*max(decay),3,est_noise,0.5*max(decay),1,0.4*max(decay),1]';
            p_min(2:8) = [0.34*max(decay),0.05,0.01,1,0.05,1,0.05]';
            p_max(2:8) = [1.2*max(decay),5,1.5*est_noise+5,1*max(decay),5,1*max(decay),5]';
        end
        
        % initial guess, lower and upper bounds of shift parameter
        p_init(1) = 0;
        p_min(1) = -0.1;
        p_max(1) = 0.1;
        ...
        %trans = find(irf == max(irf));
        trans = 0;
        
        [p_fit,Chi_sq,sigma_p,sigma_y,corr,R2,cvg_hst] = ...
            lm(@single_decay_model,p_init,time,decay,weight,0.001*free,p_min,p_max,[nexpo,trans],fit_start,fit_end);
        
        y_hat = single_decay_model(time,p_fit,[nexpo,trans]);
        ...
        fit_result(1:8,1:2) = zeros(8,2);
        fit_result(1:(nexpo*2+2),1) = real(p_fit);
        fit_result(1:(nexpo*2+2),2) = real(sigma_p);
...
selected = get(handles.Decay_listbox,'Value');
NumOfExpo = get(handles.NumOfExpo_popupmenu,'Value');
if NumOfExpo == 1;
    n = 3;
elseif NumOfExpo == 2;
    n = 5;
...
cvg_hst = handles.decay_struct{selected}.cvg_hst;
