function [pmean,psig,psampled] = FLIMMHSampler(decay_struct,Nsample,Nburn)

% [pmean,psig,psampled] = FLIMMHSampler(decay_struct,Nsample,Nburn)
...
fit_start = fit_region(1);
fit_end = fit_region(2);
...
while n<=Nsample+Nburn
    ...
    alpha = calcAlpha(time,decay,time_irf,irf,pcurrent,ptry,pmin,pmax,dp,nexpo,fitvar,fit_start,fit_end,refcurve,laserT);
...
function alpha = calcAlpha(time,data,time_irf,irf,pcurrent,ptry,pmin,pmax,dp,nexpo,fitvar,fit_start,fit_end,refcurve,laserT)
param_mat = [pcurrent,ptry];
lik = calclikelihood(time,data,time_irf,irf,param_mat,nexpo,fitvar,fit_start,fit_end,refcurve,laserT);
...
if free(1)
    ...
    ShiftProb_pcurrent = zeros(length(shift),1);
    ...
    ShiftProb_ptry = zeros(length(shift),1);
