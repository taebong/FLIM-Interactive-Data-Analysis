function [A,tau1,f,tau2,Chisq,counts] = flimImageFit(flimimage,time,irf,time_irf,binning,downsize,thres,fitsetting,fitmethod,nexpo,fitstart,fitend,noisefrom,noiseto,prior)
...
% binning: binning factor (moving sum)
% downsize: downsampling factor
...
%binning
flimimage = downsamp3d(flimimage,[1,downsize,downsize]);
[flimimage,W] = ndnanfilter(flimimage,@rectwin,[0,binning,binning]);
flimimage = flimimage*sum(W(:));
...
for y = 1:ysize
    for x= 1:xsize
        ...
        if counts(y,x) > thres            
            ...
            decay_struct.fit_result = fitsetting;
            fitted_decay_struct = ...
                DecayFLIMFit(decay_struct,fitmethod,nexpo,...
